...
def run_gui():
    ...
    tk.Entry(root, textvariable=save_path_var, state='readonly', width=50).grid(row=5, column=1, padx=10, pady=10)

    tk.Button(root, text="Select Location", command=select_save_location).grid(row=6, column=0, columnspan=2, pady=10)
    tk.Button(root, text="Start Scraping", bg="orange", fg="white", command=start_scraping).grid(row=7, column=0, columnspan=2, pady=20, ipady=5)
